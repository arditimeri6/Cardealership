...
class DoorsController extends Controller
{
    ...
    {
        return view('tables.door');
    }
    public function getData()
    {
        return Datatables::of(DB::table('doors')->get())
        ->addColumn('actions', '
    ...
    public function store(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'amount' => 'required|numeric|unique:doors',
        ], [
            'amount.required' => 'Doors number is required',
            'amount.unique' => 'This door number is already registered',
            'amount.numeric' => 'Doors number needs to be a number',
        ]);
        // dd($validation);
        $error_array = array();
        $success_output = '';
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($validation->fails()) {
            ...
            DB::table('doors')->where('id', $id)->update([
                'amount' => $request->amount,
                'updated_at' => Carbon::now()
            ]);
        }
    ...
    {
        DB::table('doors')->where('id', $id)->delete();
    }
}
