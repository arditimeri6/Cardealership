...
class HpsController extends Controller
{
    ...
    {
        return view('tables.hp');
    }
    public function getData()
    {
        return Datatables::of(DB::table('hps')->get())
        ->addColumn('actions', '
    ...
    public function store(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'amount' => 'required|numeric|unique:hps',
        ], [
            'amount.required' => 'Horsepower number is required',
            'amount.unique' => 'This horsepower number is already registered',
            'amount.numeric' => 'Horsepower number needs to be a number',
        ]);
        // dd($validation);
        $error_array = array();
        $success_output = '';
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($validation->fails()) {
            ...
            DB::table('hps')->where('id', $id)->update([
                'amount' => $request->amount,
                'updated_at' => Carbon::now()
            ]);
        }
    ...
    {
        DB::table('hps')->where('id', $id)->delete();
    }
}
