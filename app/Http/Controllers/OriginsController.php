...
class OriginsController extends Controller
{
    ...
    {
        return view('tables.origin');
    }
    public function getData()
    {
        return Datatables::of(DB::table('origins')->get())
        ->addColumn('actions', '
    ...
    public function store(Request $request)
    {
        $validation = Validator::make($request->all(), [
            ...
            'name.required' => 'Origin is required',
            'name.unique' => 'Origin is already registered',
        ]);
        // dd($validation);
        $error_array = array();
        $success_output = '';
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($validation->fails()) {
            ...
            DB::table('origins')->where('id', $id)->update([
                'name' => $request->name,
                'updated_at' => Carbon::now()
            ]);
        }
    ...
    {
        DB::table('origins')->where('id', $id)->delete();
    }
}
